...
public class ServidorHibrido {
    ...
    // Mapas Concurrentes: usamos Collections.synchronizedMap para que múltiples hilos puedan leer/escribir sin corromper los datos.
    ...
    public static void main(String[] args) {
        ...
        // En esta seccion manejamos el cierre del servidor inesperado (Shutdown Hook)
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("\n[SISTEMA] Detectada señal de apagado. Liberando recursos...");        
            // Aviso a los clientes TCP
            enviarATodos(">> SERVIDOR CERRÁNDOSE. Desconectando...");
            // Limpiamos listas
            mapaTCP.clear();
            mapaUDP.clear();
            ultimaActividad.clear();
            // Cierre del socket UDP global
            if (udpSocketGlobal != null && !udpSocketGlobal.isClosed()) {
                udpSocketGlobal.close();
                System.out.println("[SISTEMA] Socket UDP liberado.");
            }
            System.out.println("[SISTEMA] Servidor apagado correctamente.");
        }));

        // Lanzamos los hilos normales del servidor
    ...
    private static void hiloLimpiador() {
        while (true) {
            try {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (ahora - registro.getValue() > TIMEOUT_MS) {
                            ...
                            if (mapaTCP.containsKey(id)) {
                                System.out.println("Usuario [TCP] " + id + " desconectado por inactividad.");
                                mapaTCP.remove(id);
                            } else if (mapaUDP.containsKey(id)) {
                                System.out.println("Usuario [UDP] " + mapaUDP.get(id) + " desconectado por inactividad.");
                                mapaUDP.remove(id);
                            }
                ...
                // Interrupción normal durante el apagado del servidor
            } catch (Exception e) {
                System.err.println("Error en hilo limpiador: " + e.getMessage());
            }
        }
    }
    ...
    public static void iniciarTCP(int puerto) {
        try (ServerSocket serverSocket = new ServerSocket(puerto)) {
            ...
            System.err.println("Error al iniciar puerto TCP: " + e.getMessage());
        }
    }
    ...
    public static void iniciarUDP(int puerto) {
        try {
            ...
            while (true) {
                ...
                String txt = new String(paquete.getData(), 0, paquete.getLength(), StandardCharsets.UTF_8).trim(); // Convertimos con UT8 el mensaje leyendo desde la posicion 0
                ...
                if (txt.startsWith("JOIN:")) {
                    ...
                    if (!n.matches("^[a-zA-Z0-9._]+$")) {
                        ...
                        resp = "ERROR_LLENO".getBytes();
                    } else if (nombreOcupado(n)) {
                        resp = "ERROR_YA_EXISTE".getBytes();
                    } else {
                        mapaUDP.put(id, n);
            ...
            if (!udpSocketGlobal.isClosed()) System.err.println("Error en UDP: " + e.getMessage());
        }
    }
            ...
            for (Map.Entry<String, String> entry : mapaUDP.entrySet()) { // Iteramos el mapa de clientes UDP
                if (entry.getValue().equals(para)) { // Si encontramos al cliente le enviamos el mensaje //
                    try {
                        ...
                        System.err.println("Error al enviar privado UDP");
                    }
                }
            }
        }
